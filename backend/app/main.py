...
async def lifespan(app: FastAPI):
    ...
    for attempt in range(1, 6):
        try:
            ...
            ), f"embedding dim mismatch: {len(vec)}"
...
_admin_static_dir = str(
    __import__("pathlib").Path(__file__).parent / "admin" / "static"
)
app.mount(
    "/admin/static", StaticFiles(directory=_admin_static_dir), name="admin_static"
)
