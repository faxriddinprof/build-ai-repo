...
async def list_queue(
    ...
    entries_sorted = sorted(entries, key=lambda e: (_PRIORITY_SORT.get(e.priority, 2), e.queued_at))
    return [
        {
            "id": e.id,
            ...
            "priority": e.priority,
            "wait_time": int((
                __import__("datetime").datetime.utcnow() - e.queued_at
            ).total_seconds()),
            "status": e.status,
        }
...
async def accept_call(
    ...
    if entry is None:
        raise HTTPException(status_code=404, detail="Queue entry not found or already accepted")
    ...
    return {"call_id": call.id, "queue_id": queue_id, "masked_phone": entry.masked_phone}
...
async def skip_call(
    ...
    next_entry = await queue_service.skip(db, queue_id, agent.id, body.reason, body.note)
    await event_bus.publish("supervisor", {
        "type": "queue_skipped",
        "queue_id": queue_id,
        "agent_id": agent.id,
