...
- [ ] Create `backend/requirements.txt` with pinned deps (FastAPI, uvicorn, asyncpg, sqlalchemy, alembic, pgvector, faster-whisper, litellm, pymupdf, python-jose, passlib[bcrypt], pydantic-settings, structlog, rapidfuzz, tiktoken, pytest, pytest-asyncio, httpx)
- [ ] Create `.env.example` with every key from `SPEC.md` §3
- [ ] Create `.gitignore` entries for `uploads/`, `.env`, `__pycache__`, `*.pyc`, `.venv`
- [ ] Create `backend/Dockerfile` per `SPEC.md` §20.1
- [ ] Create root `docker-compose.yml` with services: `postgres` (pgvector image), `ollama`, `litellm`, `api`
- [ ] Create `litellm_config.yaml` mapping `qwen3:8b-q4_K_M` and `nomic-embed-text` to local Ollama
...
### 1.1 App skeleton
- [ ] `app/main.py` with FastAPI instance, CORS middleware, `/healthz` endpoint
- [ ] `app/config.py` Pydantic Settings loading every key from `SPEC.md` §3
- [ ] `app/logging_config.py` structlog JSON setup with `request_id` middleware
- [ ] `app/database.py` async SQLAlchemy engine + session factory
- [ ] `app/deps.py` with `get_db`, `get_current_user`, `require_role(*roles)`

### 1.2 Models
- [ ] `app/models/base.py` declarative base
- [ ] `app/models/user.py` per `SPEC.md` §4.2
- [ ] `app/models/call.py` including intake fields + `transcript`/`summary`/`compliance_status` JSONB
- [ ] `app/models/document.py` (`Document` + `DocumentChunk` with `Vector(768)`)
- [ ] `app/models/suggestion.py` (`SuggestionLog`)

### 1.3 Migrations
- [ ] `alembic init alembic` + adjust `env.py` for async + import models for autogenerate
- [ ] First migration: `CREATE EXTENSION IF NOT EXISTS vector;` in upgrade
- [ ] Second migration: autogenerated tables + `ivfflat` index on `document_chunks.embedding`
- [ ] Verify: `alembic upgrade head` runs against the compose postgres

### 1.4 Auth
- [ ] `app/services/auth_service.py` with `hash_password`, `verify_password`, `create_access_token`, `create_refresh_token`, `decode_token`
- [ ] `app/schemas/auth.py` request/response models
- [ ] `app/routers/auth.py`: `POST /api/auth/login`, `POST /api/auth/refresh`, `GET /api/auth/me`
- [ ] Wire `oauth2_scheme` into `get_current_user`
- [ ] Implement `require_role` enforcement (test with a hardcoded admin user)

### 1.5 Admin user CRUD
- [ ] `app/schemas/user.py`
- [ ] `app/routers/admin_users.py`: `POST`, `GET (list)`, `PATCH /:id` — all `require_role("admin")`
- [ ] Seed script (`scripts/seed_admin.py`) to create the initial admin user

### 1.6 Tests
- [ ] `tests/conftest.py` with test DB fixture (separate database, migrate, drop)
- [ ] `tests/test_auth.py`: hash/verify, JWT round-trip, login flow, role enforcement (401, 403)
- [ ] `tests/test_admin_users.py`: create user as admin, deny as agent
