...
**Docs:** `../docs/quickstart.md` (setup guide) · `../docs/pipeline.md` (data flow)  
**Use:** Tick boxes as you go. Don't move to the next phase until the previous phase's "Phase exit" check passes.

**Current state:** Phases 0–4 code complete. 59/59 tests passing. WAV files + frontend remaining.
...
- [x] Docker healthcheck per `SPEC.md` §20.2 (in docker-compose.yml)
...
- [x] `tests/conftest.py` shared fixtures: test DB, session-scoped migrations
- [ ] CI: GitHub Actions workflow running `pytest` against the test DB
- [x] `docs/quickstart.md` — setup guide from zero to running
- [x] `docs/pipeline.md` — full data flow diagrams for all pipelines
...
- [ ] Record / acquire 4 demo WAV files (objection, cross-sell, compliance, intake)
